...
#    """Восстановленная оригинальная обработка XML"""
#    try:
#        
...
#            logger.error(f"Root element mismatch. {xml_filename} ->  Expected: {expected_root_tag}, Actual: {xml_root.tag}")
#            return False
#
#        # Создаем CSV файл
...
def find_child_ignore_case(element, tag, namespaces):
    """Поиск дочернего элемента без учёта регистра"""
    if element is None:
        return None
    for child in element:
        localname = child.tag.split("}")[-1]  # убираем namespace
        if localname.lower() == tag.lower():
            return child
    return None
...
def process_xml(xml_path, xsd_path):
    try:
        ...
        with csv_path.open('w', newline='', encoding='utf-8') as csvfile:
            ...
            for row_index, record in enumerate(xml_root.findall(f".//{target_element}", namespaces)):
                row = []
                for col in columns:
                    parts = col.split('-')
                    element = record
                    ...
                    try:
                        for part in parts:
                            element = find_child_ignore_case(element, part, namespaces)
                            if element is None:
